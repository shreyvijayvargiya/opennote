...
class WebMCP {
    ...
    _handleToolCall(message) {
        ...
        try {
            ...
            // Helper to wrap and send result
            const sendResult = (res) => {
                const wrappedResult = this._wrapResponse(res, 'tool');
                this._sendMessage({
                    id,
                    type: 'toolResponse',
    ...
    _handleGetPrompt(message) {
        ...
        try {
            ...
            // Helper to wrap and send result
            const sendResult = (res) => {
                const wrappedResult = this._wrapResponse(res, 'prompt');
                this._sendMessage({
                    id,
                    type: 'promptResponse',
                    result: wrappedResult
    ...
    _handleReadResource(message) {
        ...
        try {
            ...
            // Helper to wrap and send result
            const sendResult = (res) => {
                const wrappedResult = this._wrapResponse(res, 'resource', uri);
                this._sendMessage({
                    id,
                    type: 'resourceResponse',
                    result: wrappedResult
    ...
    /**
     * Wrap a response into the standardized format if needed
     * @private
     * @param {any} res - The response to wrap
     * @param {string} type - The response type (tool, prompt, or resource)
     * @param {string} uri - Optional URI for resource response
     * @returns {Object} - The wrapped response
     */
    _wrapResponse(res, type, uri = null) {
        if (!res) return res;
        ...
        switch (type) {
            case 'tool':
                ...
                break;
            case 'prompt':
                if (typeof res !== 'object' || !Array.isArray(res.messages)) {
                    return {
                        messages: [{
                            role: "user",
                            content: {
                ...
                break;
            case 'resource':
                if (typeof res !== 'object' || !Array.isArray(res.contents)) {
                    return {
                        contents: [{
                            uri: uri,
                            text: typeof res === 'string' ? res : JSON.stringify(res),
    ...
    _sendMessage(message) {
        ...
        try {
            // Add JSON-RPC 2.0 version if not present to comply with standard
            ...
            this.socket.send(JSON.stringify(jsonRpcMessage));
...
// Explicitly attach to window for script tag usage
